...
def get_historico_produtos(loja_id: int, start_date: dt.date, end_date: dt.date) -> pd.DataFrame:
    """
    Retorna o histórico por produto no período [start_date, end_date], com colunas:
      produto_id, produto (nome), total_entradas, total_saidas,
      estoque_atual, ultima_contagem, estoque_inicial.
    """
    # Converter datas para timestamps de início/fim do dia
    ...
    with get_db_connection() as conn:
        df = pd.read_sql(sql, conn, params=(loja_id, start_dt, end_dt, loja_id))
    return df
